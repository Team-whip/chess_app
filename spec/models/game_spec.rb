...
RSpec.describe Game, :type => :model do
  describe "#in_check?" do
	...
	expect(@game.in_check?(@king.x_position, @king.y_position, @king.color)).to be true
      end
	...
	expect(@game.in_check?(@king2.x_position, @king2.y_position, @king2.color)).to be false
      end
    
      it "is moving into check" do
	@king2.update_attributes(x_position: 6)
	@board.refresh(@game.id)
	expect(@game.in_check?(@king2.x_position, @king2.y_position, @king2.color)).to be true
      end
    end
  end
  ...
  describe '#legal_castle_move?' do
    ...
    context 'black king castles without a rook' do
      it 'kingside' do
 	pending "This works in the console but not for the test"
	@board.board[0][7] = nil
	@board.refresh(@game.id)
